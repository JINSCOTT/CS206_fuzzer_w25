...
tensor_sizes = [(4000, 4000), (5000, 5000), (6000, 6000), (7000, 7000), (8000, 8000), (9000, 9000), (10000, 10000), (11000, 11000)]
results = []
...
def test_fn(x, y):
    return torch.div(x, y), x / y

optimized_fn = torch.compile(test_fn)  # Compile for TorchInductor

# Run tests for different tensor sizes
for size in tensor_sizes:
    a = torch.randn(size, dtype=torch.float32, device="cpu")  # Use half precision
    b = torch.randn(size, dtype=torch.float32, device="cpu")

    # Measure execution times (eager mode) with fewer iterations
    t1 = benchmark.Timer(stmt="torch.div(a, b)", globals={"a": a, "b": b, "torch": torch}).timeit(50)
    ...
    assert torch.allclose(out_eager[0], out_optimized[0], atol=1e-3, rtol=1e-2), "Torch.div mismatch in Inductor!"
    assert torch.allclose(out_eager[1], out_optimized[1], atol=1e-3, rtol=1e-2), "/ operator mismatch in Inductor!"

    # Store results
    results.append((size, t1, t2))

# Create DataFrame for results
df = pd.DataFrame(results, columns=["Tensor Size", "torch.div Time (s)", "/ Operator Time (s)"])
# Extract execution times and print them in seconds
df["torch.div Time (s)"] = df["torch.div Time (s)"].apply(lambda x: x.mean)
...
plt.plot([str(s) for s in tensor_sizes], [r[1].mean for r in results], marker='o', label="torch.div")
plt.plot([str(s) for s in tensor_sizes], [r[2].mean for r in results], marker='s', label="/ Operator")

plt.xlabel("Tensor Size")
plt.ylabel("Execution Time (seconds)")
plt.title("Execution Time Comparison: torch.div vs / Operator")
plt.legend()
plt.grid(True)
