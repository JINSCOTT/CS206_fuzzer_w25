...
for size in tensor_sizes:
    ...
    t1 = benchmark.Timer(stmt="torch.sub(a, b)", globals={"a": a, "b": b, "torch": torch}).timeit(50)
    ...
    assert torch.allclose(out_eager[0], out_optimized[0]), "Torch.sub mismatch in Inductor!"
...
df = pd.DataFrame(results, columns=["Tensor Size", "torch.sub Time (s)", "- Operator Time (s)"])
# Extract execution times and print them in seconds
df["torch.sub Time (s)"] = df["torch.sub Time (s)"].apply(lambda x: x.mean)
...
plt.plot([str(s) for s in tensor_sizes], [r[1].mean for r in results], marker='o', label="torch.sub")
plt.plot([str(s) for s in tensor_sizes], [r[2].mean for r in results], marker='s', label="- Operator")

plt.xlabel("Tensor Size")
plt.ylabel("Execution Time (seconds)")
plt.title("Execution Time Comparison: torch.sub vs - Operator")
plt.legend()
plt.grid(True)
